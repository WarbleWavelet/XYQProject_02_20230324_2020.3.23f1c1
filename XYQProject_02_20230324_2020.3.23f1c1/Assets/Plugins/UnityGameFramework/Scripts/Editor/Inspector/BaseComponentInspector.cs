...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class BaseComponentInspector : GameFrameworkInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlayingOrWillChangePlaymode);
            {
                ...
                m_EditorResourceMode.boolValue = EditorGUILayout.BeginToggleGroup("Editor Resource Mode", m_EditorResourceMode.boolValue);
                {
                    ...
                    EditorGUILayout.PropertyField(m_EditorLanguage);// EditorLanguage
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    if ( SelectNew( textHelperSelectedIndex , m_TextHelperTypeNameIndex) )
                    {
                        m_TextHelperTypeNameIndex = textHelperSelectedIndex;
                        m_TextHelperTypeName.stringValue =   GetStringValue( textHelperSelectedIndex ,m_TextHelperTypeNames );
                    }
                    ...
                    if (SelectNew(versionHelperSelectedIndex , m_VersionHelperTypeNameIndex) )
                    {
                        m_VersionHelperTypeNameIndex = versionHelperSelectedIndex;
                        m_VersionHelperTypeName.stringValue = GetStringValue(versionHelperSelectedIndex, m_VersionHelperTypeNames);
                    }

                    int logHelperSelectedIndex = EditorGUILayout.Popup("Log Helper", m_LogHelperTypeNameIndex, m_LogHelperTypeNames);
                    if (SelectNew(logHelperSelectedIndex , m_LogHelperTypeNameIndex))
                    {
                        m_LogHelperTypeNameIndex = logHelperSelectedIndex;
                        m_LogHelperTypeName.stringValue = GetStringValue(logHelperSelectedIndex , m_LogHelperTypeNames);
                    }
                    ...
                    if (SelectNew(compressionHelperSelectedIndex ,m_CompressionHelperTypeNameIndex) )
                    {
                        m_CompressionHelperTypeNameIndex = compressionHelperSelectedIndex;
                        m_CompressionHelperTypeName.stringValue = GetStringValue(compressionHelperSelectedIndex , m_CompressionHelperTypeNames);
                    }

                    int jsonHelperSelectedIndex = EditorGUILayout.Popup("JSON Helper", m_JsonHelperTypeNameIndex, m_JsonHelperTypeNames);
                    if (SelectNew(jsonHelperSelectedIndex , m_JsonHelperTypeNameIndex)  )
                    {
                        m_JsonHelperTypeNameIndex = jsonHelperSelectedIndex;
                        m_JsonHelperTypeName.stringValue = GetStringValue(jsonHelperSelectedIndex , m_JsonHelperTypeNames );
                    }
                }
        ...
        #region private
        /// <summary>切换了不同的选项</summary>
        bool SelectNew(int selected,int cur)
        {
          
            return selected != cur;
        }

        string GetStringValue(int idx,string[] opts)
        {
           return idx <= 0 ? null : opts[idx];
        }

        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginVertical(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal(); //自己加的
                    {
                        EditorGUILayout.LabelField("说明", GUILayout.Width(160f));
                        EditorGUILayout.LabelField("01 Assets下保证有StreamingAssets；02 退出对要打包的资源的使用；03 脚本注意隔离开UNITY_EDITOR");
                    }
                    EditorGUILayout.EndHorizontal();
                }

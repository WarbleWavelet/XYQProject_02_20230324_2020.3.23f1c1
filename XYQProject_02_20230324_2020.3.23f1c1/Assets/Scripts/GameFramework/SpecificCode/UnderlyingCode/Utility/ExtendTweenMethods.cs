...
public static class ExtendTweenMethods 
{
    ...
    public class Tween
    {
        ...
        /// <summary>动画执行逻辑的协程</summary>
        public Coroutine tweenCoroutine;
        /// <summary>动画循环次数</summary>
        public int loopCnt;
        /// <summary>当前循环到的次数</summary>
        public int currentLoopCnt;
        /// <summary>起始值</summary>
        public Vector3 originalValue;
        /// <summary>目标值</summary>
        public Vector3 targetValue;
        /// <summary>根据调用方法不同进行不同的处理（比如初始化）</summary>
        public TweenMethodType tweenMethodType;
        /// <summary>作用的transform对象</summary>
        ...
        public Tween(TweenMethodType type,
            Transform trans,
            Vector3 targetVal,
            float tweenTime,
            int loopCout=1
        ...
        public void Kill()
        {
            if (GameStartInstance.Instance == null)
            {
                return;
            }
            GameStartInstance.Instance.StopCoroutine(tweenCoroutine);
        }
        /// <summary>
        /// 播放动画
        /// </summary>
    ...
    {
        for (tween.currentLoopCnt = 0; tween.currentLoopCnt < tween.loopCnt; tween.currentLoopCnt++)
        {
            tween.ResetState();
            //动画总体插值
    ...
    public static Tween DoMove(this Transform transform,Vector3 target,float time,int loop=1)
    {
        ...
        Coroutine coroutine =GameStartInstance.Instance.StartCoroutine(GameStartInstance.Instance.TweenLogicCoroutine(tween));
        tween.SetCoroutine(coroutine);
        return tween;
    }

    public static Tween DoMove(this Transform transform,MonoBehaviour monoBehaviour, Vector3 target, float time, int loop = 1)
    {
        Tween tween = new Tween(TweenMethodType.DOMOVE, transform, target, time, loop);
        Coroutine coroutine = monoBehaviour.StartCoroutine(monoBehaviour.TweenLogicCoroutine(tween));
        tween.SetCoroutine(coroutine);
        return tween;
    }
    #endregion
